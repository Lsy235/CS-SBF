...
def preprocess_item(item):
    ...
    # print(f"N: {N}, max: {max(edge_index[0, :])}, {max(edge_index[1, :])}, min: {min(edge_index[0, :])}, {min(edge_index[1, :])}")
    ...
    # attn_edge_type = attn_edge_type.to(dtype=torch.int32)
    ...
    # edge_input = edge_input.to(dtype=torch.int32)

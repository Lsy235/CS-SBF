    ...
    def process(self):
        for i, m in enumerate(['train', 'val', 'test']):
            ...
            for j, raw_file in enumerate(dataList):
                ...
                for edge in G.edges(data=True):
                    pos_u, pos_v = np.array([G.nodes[edge[0]]['feature'][0], G.nodes[edge[0]]['feature'][1]]), np.array(
                        [G.nodes[edge[1]]['feature'][0], G.nodes[edge[1]]['feature'][1]])
                    if(edge[0] > node_features.shape[0]):
                        continue
                    for w in G.neighbors(edge[0]):
                        if (w != edge[1] and w<node_features.shape[0]):
                ...
                edge_index_b = torch.tensor(edge_index_b[:edge_index.shape[1]]).T
                edge_attr_b = torch.tensor(edge_attr_b[:edge_index.shape[1]])
                # x_b = torch.ones([edge_index_b.shape[1], 1])
                label = labels[j]
                ...
                print(f"lNum No.{lNum}, "
                      ...
                      f"x_b:{x_b.shape},"
                      f"edge_index_b:{edge_index_b.shape},"
                      f"edge_attr_b:{edge_attr_b.shape}")
            print(f"{m} lNum: {lNum}")
...
#             f"x_b:{graph.x_b.size()},"
#             f"edge_index_b:{graph.edge_index_b.size()},"
#             f"edge_attr_b:{graph.edge_attr_b.size()},"
